...
. (Join-Path $PSScriptRoot "\Utilities\FileUtils.ps1")
...
if (-not (Get-Module -Name $ExistingAuthModule.Name -ListAvailable)) {
...
$ModulesToGenerate | ForEach-Object {
    ...
    Copy-ModuleTemplate -Destination $ModuleConfig -TemplatePath (Join-Path $ScriptRoot "\Templates\module.md") -ModuleName $Module
    ...
    @("v1.0", "beta") | ForEach-Object {
        ...
        $ModuleName = ($ApiVersion -eq "beta" ? "$Module.Beta" : $Module)
        ...
        elseif ($VersionState.Equals([VersionState]::Valid) -or $VersionState.Equals([VersionState]::NotOnFeed) -or $SkipVersionCheck) {
            ...
            try {
                ...
                Write-Host -ForeGroundColor Green "Generated '$ModuleFullName' successfully."
            }
            catch {
                Write-Error $_
            }
